...
- [~] **Pas de pagination sur plusieurs listes à fort volume** *(audité le 2026-08-18, 10/13 endpoints migrés le 2026-08-18)* — 13 endpoints identifiés qui chargeaient la liste complète, avec recherche/filtres faits côté client sur la totalité chargée. Brief envoyé (`BACKEND_PROMPT_PAGINATION.md`), retour reçu (`FRONTEND_PROMPT_PAGINATION.md`) : le backend a ajouté `page`/`limit`/`search`/filtres en mode rétrocompatible (sans ces params, comportement legacy inchangé).
  ...
  - **Bug corrigé en cours de route** : tous les écrans migrés crashaient (`Cannot read properties of undefined`) si l'état Redux persisté en `localStorage` datait d'avant ce chantier — ajout de valeurs par défaut (`[]`/meta neutre) à la déstructuration sur les 10 écrans.
  ...
  - **`dossier-commun` (1 endpoint) toujours bloqué** : l'écran `ListeDossierByModule.tsx` filtre par module + statut du collaborateur, une logique que le contrat backend actuel ne couvre pas (seulement `search`/`statut` du dossier). Complément de brief à transmettre → `BACKEND_PROMPT_PAGINATION_DOSSIER_COMMUN.md`.
  - **2 endpoints restants, non traités** (pas de blocage identifié, juste pas encore fait) : `sav/rappel-ticketing` et `sav/sondage` sont en réalité déjà migrés (inclus dans les 10) — reste seulement à vérifier un jour si le bug de routage `:id` vs sous-routes signalé par le backend impacte d'autres parties de l'app que `TabRappel.tsx`/`TabSondage.tsx` (vérifié non impactés pour ces deux écrans).
  - Cas à surveiller sans urgence (à trancher avec le backend, phase 2 du brief) : `fournisseurs`, `users`, `miles-transaction/client/{id}`.
  - `npx tsc -b --noEmit` et `npm run build` : 0 erreur après migration.
