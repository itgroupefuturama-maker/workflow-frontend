        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center">
              ...
              <button
                onClick={onCloseCreate} // Adapté : au lieu de closeCreateModal
                className="text-slate-500 hover:text-slate-800"
                disabled={isCreating}
              ><FiX size={24} /></button>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex justify-end gap-4 pt-4">
                <button
                  onClick={onCloseCreate} // Adapté
                  disabled={isCreating}
                  className="px-6 py-2.5 bg-white border border-slate-300 text-slate-700 rounded-xl"
                >Annuler</button>
                <button
                  onClick={onConfirmCreate} // Adapté : au lieu de handleCreateEntete
                ...
                >
                  {isCreating ? (
                    <><span className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full"></span> Création...</>
                  ) : 'Créer'}
                </button>
              </div>
            </div>
