...
export const generateAttestationPdf = (
  ...
  const layout = makeLayout('landscape');

  drawWatermark(doc, design, layout);
  drawHeader(doc, cursor, 'ATTESTATION DE VOYAGE', design, logo, layout);
...
function _renderParPassager(
  ...
  parPassager.forEach(({ info, lignes }) => {
    ...
    drawTable(doc, cursor, design,
      [
        { label: 'Entête',      width: 25 },
        { label: 'Réf.',        width: 35 },
        { label: 'N° Vol',      width: 35 },
        { label: 'Itinéraire',  width: 72 },
        { label: 'Classe',      width: 28 },
        { label: 'Départ',      width: 30 },
        { label: 'Arrivée',     width: 30 },
      ],
...
function _drawLigneBlock(
  ...
  if (ligne.attestationPassager.length > 0) {
    drawTable(doc, cursor, design,
      [
        { label: 'Nom',          width: 42 },
        { label: 'Prénom',       width: 42 },
        { label: 'Nationalité',  width: 38 },
        { label: 'Type doc',     width: 34 },
        { label: 'N° doc',       width: 36 },
        { label: 'Délivrance',   width: 32 },
        { label: 'Validité',     width: 31 },
      ],
